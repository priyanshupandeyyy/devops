...
Docker Registries
A Registry is a specialized server that stores and distributes Docker images. A Repository is a collection of specific versions (tags) of one image inside that registry.
...
-it : Runs the container in interactive mode -d : Runs the container in the background --name : Specifies a name for the container --rm : Automatically removes the container when it exists -p : Maps a host port to a container port -e / --env : Sets an env variable inside the container -v : Mounts a host volume inside the container
